...
class KaggleVegetable(DatasetBaseMT):
    
    dataset_dir = "kaggle_vegetable"
    taskid2type = ['sl']
    ...
    def __init__(self, cfg):
        ...
        self.split_path = os.path.join(self.dataset_dir, "split_dk_KaggleVegetable.json")
        ...
        if os.path.exists(self.split_path):
            ...
            train, val, test, taskid2cname = [], [], [], None
            for split in ["train", "test", "validation"]:
                ret = DTD.read_and_split_data(os.path.join(self.image_dir, split))
                train.extend(ret[0])
                val.extend(ret[1])
                test.extend(ret[2])
                taskid2cname = ret[3]
        ...
        super().__init__(taskid2tname, taskid2cname, taskid2desc, self.taskid2type, self.taskid2metric, train_x=train, val=val, test=test)
