...
for SEED in 1 2 3
do
    DIR=outputd/${DATASET}/${TRAINER}/${CFG}_${SHOTS}shots/cnctx${CLASS_N_CTX}_ccsc${CLASS_CSC}_tnctx${TASK_N_CTX}_tcsc${TASK_CSC}_nctx${N_CTX}_csc${CSC}_type${PG_TYPE}_ratio${PG_RATIO}_csr${CSR}/seed${SEED}
FEAT_FILE=${DATASET}"+"${CFG}
    if [ -d "$DIR" ]; then
        ...
        TRAINER.MTCOOP.CLS_SAMPLE_RATE ${CSR} \
