    ...
    for subdir in subdirs:
        ...
        with open(fpath, "r") as f:
            ...
            for line in lines:
                ...
                if "* task:" in line:
                    taskid = int(re.findall(r"\d+", line)[0])
                    acc = float(line.split("metric:")[-1][:-1].strip())
                    task_res[taskid].append(acc)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--keyword", default="overall_metric", type=str, help="which keyword to extract"
    )
